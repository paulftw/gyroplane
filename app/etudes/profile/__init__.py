...
"""
    Extends gaesessions.Session to add session.profile cached lazy property.
"""
class ProfileSession(gaesessions.Session):
    
    def __init__(self, *args, **kwargs):
        gaesessions.Session.__init__(self, *args, **kwargs)
        self._profile = None
    
    @cached_property
    def profile(self):
        if not self._profile:
            self._profile = gaeprofiles.current_profile()
        return self._profile
    
    def is_profile_dirty(self):
        if not self._profile:
            return False
        else:
            return self._profile.is_dirty()
...
class ProfilesBlueprint(Blueprint):
    ...
    def load_or_create_profile(self, key):
        if Profile.is_generated_key(key):
...
class Profile(db.Expando):
    ...
    @staticmethod
    def is_generated_key(key):
        """Returns true if given key was generated by GAE."""
        return isinstance(key, (int, long))
    ...
    def is_guest(self):
        # Only guest profiles use GAE generated ids. 
        return self.is_generated_key(self.get_id())
